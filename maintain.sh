...
awk '/^== port ==/{f=1;next} /^== /{f=0} f{print}' "$DEST/health.txt" | grep -qv '"xray"' && die ":443 not owned by xray"
awk '/^== port ==/{f=1;next} /^== /{f=0} f{print}' "$DEST/health.txt" | grep -o '"[^"]*"' | grep -qv '"xray"' && die ":443 not owned by xray"
