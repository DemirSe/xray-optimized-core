...
func (v *Address) UnmarshalJSON(data []byte) error {
	...
	if strings.HasPrefix(rawStr, "env:") {
		rawStr = platform.NewEnvFlag(rawStr[4:]).GetValue("")
	}
...
func parseStringPort(s string) (net.Port, net.Port, error) {
	if strings.HasPrefix(s, "env:") {
		s = platform.NewEnvFlag(s[4:]).GetValue("")
	}
...
// A single negative integer parses as-is (sentinel); ranges are non-negative "A-B".
// Value is exchanged if From > To.
...
func (v *Int32Range) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &str); err == nil {
		...
		if err == nil {
			v.From, v.To = int32(from), int32(to)
			if v.From > v.To {
...
// Parse range in string, e.g. "114-514" "114514" ""(return 0).
// A single integer (including a negative sentinel like "-1") parses as-is;
// ranges are non-negative "A-B".
